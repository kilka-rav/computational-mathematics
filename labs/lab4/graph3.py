...
_x, _y = "x_cub.txt", "y_cub.txt"                 

x = np.genfromtxt(_x, dtype = 'double')
y = np.genfromtxt(_y, dtype = 'double')

_x_two = "x_newton.txt"
x_two = np.genfromtxt(_x_two, dtype = 'double')
y_two = np.log(x_two) + np.sqrt(1 + x_two)


spline = BuildSpline(x, y)
xs = np.arange(1, 16, 0.01)
fs = [Cubic(spline, x) for x in xs]
fig, ax = plt.subplots()
plt.scatter(x, y, color="green", label="origins")
plt.plot(x_two, y_two, color = 'b', label = 'orig_func', linestyle = '-')
